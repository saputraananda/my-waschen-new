...
import { Receipt, ArrowLeft, CreditCard, Save, Wallet, Coins, Upload, Camera, X, Loader2 } from 'lucide-react';
...
export default function Payment({
  ...
  // Nota belum terbit saat bayar, jadi baris kedua = label bukti + metode bayar.
  const proofLines = (date) => buildQcPhotoLines({
    orderNo: 'Bukti Bayar',
    stage: mainCategory || 'Pembayaran',
    date
  });

  const pickProofFile = async (file) => {
    if (!file) return;
    ...
    try {
      setPaymentProofFile(await burnQcPhoto(file, { orderNo: 'Bukti Bayar', stage: mainCategory || 'Pembayaran' }));
    } catch {
      showAlert({
        title: 'Foto tidak valid',
        message: 'Pilih foto jpeg, png, atau webp.',
        type: 'error'
      });
    }
  };
  ...
  const hasProof = Boolean(paymentProofFile);
  const canSubmit = isOutstanding
    || (isCash && isLunas && (isMemberBalanceMethod || (paidAmountNum >= calculations.grandTotal && !isUnderpaidLunas)))
    || (isCash && isDP && paidAmountNum > 0 && paidAmountNum < calculations.grandTotal)
    || (hasProof && isLunas && (isMemberBalanceMethod || (paidAmountNum >= calculations.grandTotal && !isUnderpaidLunas)))
    || (hasProof && isDP && paidAmountNum > 0 && paidAmountNum < calculations.grandTotal);
              ...
              <>
                ...
                {isCash ? (
                  <p className="text-[11px] font-bold text-slate-500 leading-relaxed">
                    Pembayaran tunai: bukti diunggah di halaman selesai, setelah nota dicetak dan ditandatangani customer.
                  </p>
                ) : (
                <div>
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-1.5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    ...
                    <button
                      ...
                      className="flex flex-col items-center justify-center w-full p-4 border-2 border-dashed border-[#e0e0e0] rounded-2xl cursor-pointer hover:border-[#5f1340]/40 hover:bg-[#5f1340]/[0.02] transition-all"
                    >
